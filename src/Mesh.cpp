...
void Mesh::sendMeshMsg(uint8_t* msg, uint8_t msgSize, uint8_t stoneId) {
	...
	memcpy(&global_msg.payload[CMD_SIZE + meshHeaderSize + meshHeaderSendSize], msg, msgSize);

	global_msg.length = CMD_SIZE + meshHeaderSize + meshHeaderSendSize + msgSize;
...
uint8_t Mesh ::readMeshMsg(uint8_t** msgPtr, uint8_t* stoneIdPtr) {
	...
	microapp_mesh_read_t* read_mesh_cmd_header = (microapp_mesh_read_t*)&global_msg.payload[CMD_SIZE + meshHeaderSize];
